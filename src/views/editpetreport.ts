...
import type { UpdatePetReport } from "../state";
import Dropzone from "dropzone";
import mapboxgl from "mapbox-gl";
import "mapbox-gl/dist/mapbox-gl.css";

class EditPetReport extends HTMLElement {
  ...
  render() {
    ...
    <div class="view">
      ...
      <div class="main-cont">
        <div class="container">
          ...
          <h2 class="subtitle">Ingresá solo los datos que quieras actualizar. Los demás se mantendrán igual.</h2>
          <form>
            <div class="group">
            ...
            <p class="error">Ingresá minimo un dato para continuar</p>
    ...
    const deleteReport = this.shadowDom.querySelector(".delete") as HTMLElement;
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const obj: UpdatePetReport = {};
      const params = new URLSearchParams(window.location.search);
      const id = Number(params.get("id"));

      if (!name && !imageURL && !this.lng && !this.lat) {
      ...
      if (this.lat && this.lng) {
        obj.lat = this.lat;
        obj.lng = this.lng;
        const location = await this.getReadableLocation(this.lat, this.lng);
        obj.location = location;
      }
      console.log(obj);
      const res = await state.updatePetReport(obj);

      if (res !== true) {
        alert("Error al actualizar el reporte");
      } else {
        Router.go("/reportedpets");
      }
    });

    deleteReport.addEventListener("click", async () => {
      const params = new URLSearchParams(window.location.search);
      const id = Number(params.get("id"));
      const res = await state.deletePetReport(id);
      if (res === true) {
        Router.go("/reportedpets");
      }
    });
...
customElements.define("edit-pet-report", EditPetReport);
